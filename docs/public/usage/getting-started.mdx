...
description: "Learn how claude-recall works automatically in the background"
---

# Getting Started with claude-recall

## Automatic Operation

claude-recall works automatically once installed. No manual intervention required!
...
Every time Claude uses a tool, claude-recall captures it:
...
Context is stored in SQLite database at `~/.claude-recall/claude-recall.db`.
...
claude-recall supports sessions that span multiple user prompts:
...
claude-recall provides MCP tools for querying your project history. Simply ask naturally:
