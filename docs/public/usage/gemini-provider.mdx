...
claude-recall supports Google's Gemini API as an alternative to the Claude Agent SDK for extracting observations from your sessions. This can significantly reduce costs since Gemini offers a generous free tier.
...
|---------|--------|---------|-------------|
| `CLAUDE_RECALL_PROVIDER` | `claude`, `gemini` | `claude` | AI provider for observation extraction |
| `CLAUDE_RECALL_GEMINI_API_KEY` | string | — | Your Gemini API key |
| `CLAUDE_RECALL_GEMINI_MODEL` | `gemini-2.5-flash-lite`, `gemini-2.5-flash`, `gemini-3-flash` | `gemini-2.5-flash-lite` | Gemini model to use |
| `CLAUDE_RECALL_GEMINI_BILLING_ENABLED` | `true`, `false` | `false` | Skip rate limiting if billing is enabled on Google Cloud |
...
```json
{
  ...
  "CLAUDE_RECALL_GEMINI_API_KEY": "your-api-key-here",
  "CLAUDE_RECALL_GEMINI_MODEL": "gemini-2.5-flash-lite",
  "CLAUDE_RECALL_GEMINI_BILLING_ENABLED": "true"
}
```
...
If Gemini is selected but encounters errors, claude-recall automatically falls back to the Claude Agent SDK:
...
Either:
- Set `CLAUDE_RECALL_GEMINI_API_KEY` in `~/.claude-recall/settings.json`, or
...
claude-recall enforces these limits automatically with built-in delays between requests. Processing may be slower but stays within limits.
...
<Tip>
**Recommended**: Enable billing on your Google Cloud project to unlock much higher rate limits. You won't be charged unless you exceed the generous free quota. This allows claude-recall to process observations instantly instead of waiting between requests.
</Tip>
