...
claude-recall provides persistent memory across sessions through **4 MCP tools** that follow a token-efficient **3-layer workflow pattern**.

## Overview

Instead of fetching all historical data upfront (expensive), claude-recall uses a progressive disclosure approach:
...
**MCP Server:** Located at `~/.claude/plugins/marketplaces/nhevers/extension/runtime/search-server.cjs`

**Worker Service:** Express API on port 37777, managed by Bun

**Database:** SQLite FTS5 full-text search on `~/.claude-recall/claude-recall.db`
