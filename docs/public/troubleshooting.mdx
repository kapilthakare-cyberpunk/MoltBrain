...
description: "Common issues and solutions for claude-recall"
---
...
1. Check browser localStorage is enabled:
   ```javascript
   // In browser console
   localStorage.getItem('claude-recall-settings')
   ```
...
3. Clear localStorage and try again:
   ```javascript
   // In browser console
   localStorage.removeItem('claude-recall-settings')
   ```
...
1. Clear install cache:
   ```bash
   rm ~/.claude/plugins/marketplaces/nhevers/.install-version
   ```
...
3. Check version marker:
   ```bash
   cat ~/.claude/plugins/marketplaces/nhevers/.install-version
   cat ~/.claude/plugins/marketplaces/nhevers/package.json | grep version
   ```
...
3. Verify database has observations:
   ```bash
   sqlite3 ~/.claude-recall/claude-recall.db "SELECT COUNT(*) FROM observations;"
   ```
...
3. Check hook permissions:
   ```bash
   ls -la extension/runtime/*.js
   ```

4. Verify lifecycle.json is valid JSON:
   ```bash
   cat extension/lifecycle/lifecycle.json | jq .
   ```
...
1. Check if summaries exist:
   ```bash
   sqlite3 ~/.claude-recall/claude-recall.db "SELECT COUNT(*) FROM session_summaries;"
   ```
...
1. Increase timeout in `extension/lifecycle/lifecycle.json`:
   ```json
   {
...
4. Optimize database:
   ```bash
   sqlite3 ~/.claude-recall/claude-recall.db "VACUUM;"
   ```
...
3. Check package.json exists:
   ```bash
   ls -la ~/.claude/plugins/marketplaces/nhevers/package.json
   ```
...
1. Check FTS5 tables exist:
   ```bash
   sqlite3 ~/.claude-recall/claude-recall.db "SELECT name FROM sqlite_master WHERE type='table' AND name LIKE '%_fts';"
   ```
...
3. Check triggers exist:
   ```bash
   sqlite3 ~/.claude-recall/claude-recall.db "SELECT name FROM sqlite_master WHERE type='trigger';"
   ```
...
1. Check MCP configuration:
   ```bash
   cat extension/.mcp.json
   ```

2. Verify search server is built:
   ```bash
   ls -l extension/runtime/search-server.cjs
   ```
...
2. Verify FTS5 tables populated:
   ```bash
   sqlite3 ~/.claude-recall/claude-recall.db "SELECT COUNT(*) FROM observations_fts;"
   ```
...
**Symptoms**: `/plugin install claude-recall` fails.
...
3. Verify installation:
   ```bash
   ls -la ~/.claude/plugins/marketplaces/nhevers/
   ```
...
**Solution**: Check `~/.claude-recall/` exists and is writable.
...
2. **Create issue**: [GitHub Issues](https://github.com/nhevers/claude-recall/issues)
