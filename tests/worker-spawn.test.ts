...
const TEST_DATA_DIR = path.join(homedir(), '.claude-recall-test');
const TEST_PID_FILE = path.join(TEST_DATA_DIR, 'worker.pid');
const WORKER_SCRIPT = path.join(__dirname, '../extension/runtime/engine-runtime.cjs');
...
describe('Windows-specific behavior', () => {
  ...
  afterEach(() => {
    ...
    delete process.env.CLAUDE_RECALL_MANAGED;
  });
  ...
  it('should detect Windows managed worker mode correctly', () => {
    ...
    const isManaged = process.env.CLAUDE_RECALL_MANAGED === 'true';
