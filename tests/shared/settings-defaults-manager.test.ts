...
describe('SettingsDefaultsManager', () => {
  ...
  describe('loadFromFile', () => {
    describe('file does not exist', () => {
      ...
      it('should write pretty-printed JSON (2-space indent)', () => {
        ...
        expect(content).toContain('  "CLAUDE_RECALL_MODEL"');
      });
    ...
    describe('file exists with valid content', () => {
      it('should return parsed content when file has valid JSON', () => {
        const customSettings = {
          CLAUDE_RECALL_MODEL: 'custom-model',
          CLAUDE_RECALL_WORKER_PORT: '12345',
        };
        writeFileSync(settingsPath, JSON.stringify(customSettings));

        const result = SettingsDefaultsManager.loadFromFile(settingsPath);

        expect(result.CLAUDE_RECALL_MODEL).toBe('custom-model');
        expect(result.CLAUDE_RECALL_WORKER_PORT).toBe('12345');
      });

      it('should merge file settings with defaults for missing keys', () => {
        // Only set one value, defaults should fill the rest
        const partialSettings = {
          CLAUDE_RECALL_MODEL: 'partial-model',
        };
        ...
        expect(result.CLAUDE_RECALL_MODEL).toBe('partial-model');
        // Other values should come from defaults
        expect(result.CLAUDE_RECALL_WORKER_PORT).toBe(defaults.CLAUDE_RECALL_WORKER_PORT);
        expect(result.CLAUDE_RECALL_WORKER_HOST).toBe(defaults.CLAUDE_RECALL_WORKER_HOST);
        expect(result.CLAUDE_RECALL_LOG_LEVEL).toBe(defaults.CLAUDE_RECALL_LOG_LEVEL);
      });

      it('should not modify existing file when loading', () => {
        const customSettings = {
          CLAUDE_RECALL_MODEL: 'do-not-change',
      ...
      it('should handle all settings keys correctly', () => {
        ...
        fullSettings.CLAUDE_RECALL_MODEL = 'all-keys-model';
        fullSettings.CLAUDE_RECALL_PROVIDER = 'gemini';
        writeFileSync(settingsPath, JSON.stringify(fullSettings));

        const result = SettingsDefaultsManager.loadFromFile(settingsPath);

        expect(result.CLAUDE_RECALL_MODEL).toBe('all-keys-model');
        expect(result.CLAUDE_RECALL_PROVIDER).toBe('gemini');
      });
    });
    ...
    describe('nested schema migration', () => {
      it('should migrate old nested { env: {...} } schema to flat schema', () => {
        const nestedSettings = {
          env: {
            CLAUDE_RECALL_MODEL: 'nested-model',
            CLAUDE_RECALL_WORKER_PORT: '54321',
          },
        };
        ...
        expect(result.CLAUDE_RECALL_MODEL).toBe('nested-model');
        expect(result.CLAUDE_RECALL_WORKER_PORT).toBe('54321');
      });

      it('should auto-migrate file from nested to flat schema', () => {
        const nestedSettings = {
          env: {
            CLAUDE_RECALL_MODEL: 'migrated-model',
          },
        };
        writeFileSync(settingsPath, JSON.stringify(nestedSettings));
        ...
        expect(parsed.CLAUDE_RECALL_MODEL).toBe('migrated-model');
      });
    });
    ...
    describe('edge cases', () => {
      ...
      it('should ignore unknown keys in file', () => {
        const settingsWithUnknown = {
          CLAUDE_RECALL_MODEL: 'known-model',
        ...
        expect(result.CLAUDE_RECALL_MODEL).toBe('known-model');
        expect((result as Record<string, unknown>).UNKNOWN_KEY).toBeUndefined();
      });

      it('should handle file with BOM', () => {
        const bom = '\uFEFF';
        const settings = { CLAUDE_RECALL_MODEL: 'bom-model' };
  ...
  describe('getAllDefaults', () => {
    ...
    it('should include all expected keys', () => {
      ...
      // Core settings
      expect(defaults.CLAUDE_RECALL_MODEL).toBeDefined();
      expect(defaults.CLAUDE_RECALL_WORKER_PORT).toBeDefined();
      expect(defaults.CLAUDE_RECALL_WORKER_HOST).toBeDefined();

      // Provider settings
      expect(defaults.CLAUDE_RECALL_PROVIDER).toBeDefined();
      expect(defaults.CLAUDE_RECALL_GEMINI_API_KEY).toBeDefined();
      expect(defaults.CLAUDE_RECALL_OPENROUTER_API_KEY).toBeDefined();

      // System settings
      expect(defaults.CLAUDE_RECALL_DATA_DIR).toBeDefined();
      expect(defaults.CLAUDE_RECALL_LOG_LEVEL).toBeDefined();
    });
  });

  describe('get', () => {
    it('should return default value for key', () => {
      expect(SettingsDefaultsManager.get('CLAUDE_RECALL_MODEL')).toBe('claude-sonnet-4-5');
      expect(SettingsDefaultsManager.get('CLAUDE_RECALL_WORKER_PORT')).toBe('37777');
    });
  });

  describe('getInt', () => {
    it('should return integer value for numeric string', () => {
      expect(SettingsDefaultsManager.getInt('CLAUDE_RECALL_WORKER_PORT')).toBe(37777);
      expect(SettingsDefaultsManager.getInt('CLAUDE_RECALL_CONTEXT_OBSERVATIONS')).toBe(50);
    });
  });

  describe('getBool', () => {
    it('should return true for "true" string', () => {
      expect(SettingsDefaultsManager.getBool('CLAUDE_RECALL_CONTEXT_SHOW_READ_TOKENS')).toBe(true);
    });

    it('should return false for non-"true" string', () => {
      expect(SettingsDefaultsManager.getBool('CLAUDE_RECALL_CONTEXT_SHOW_LAST_MESSAGE')).toBe(false);
    });
  });
});
