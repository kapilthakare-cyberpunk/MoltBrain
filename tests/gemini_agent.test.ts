...
import { GeminiAgent } from '../src/core/worker/GeminiAgent';
import { DatabaseManager } from '../src/core/worker/DatabaseManager';
import { SessionManager } from '../src/core/worker/SessionManager';
import { ModeManager } from '../src/core/domain/ModeManager';
...
describe('GeminiAgent', () => {
  ...
  beforeEach(() => {
    ...
    loadFromFileSpy = spyOn(SettingsDefaultsManager, 'loadFromFile').mockImplementation(() => ({
      ...
      CLAUDE_RECALL_GEMINI_API_KEY: 'test-api-key',
      CLAUDE_RECALL_GEMINI_MODEL: 'gemini-2.5-flash-lite',
      CLAUDE_RECALL_GEMINI_RATE_LIMITING_ENABLED: rateLimitingEnabled,
      CLAUDE_RECALL_DATA_DIR: '/tmp/claude-recall-test',
    }));

    getSpy = spyOn(SettingsDefaultsManager, 'get').mockImplementation((key: string) => {
      if (key === 'CLAUDE_RECALL_GEMINI_API_KEY') return 'test-api-key';
      if (key === 'CLAUDE_RECALL_GEMINI_MODEL') return 'gemini-2.5-flash-lite';
      if (key === 'CLAUDE_RECALL_GEMINI_RATE_LIMITING_ENABLED') return rateLimitingEnabled;
      if (key === 'CLAUDE_RECALL_DATA_DIR') return '/tmp/claude-recall-test';
  ...
  it('should respect rate limits when rate limiting enabled', async () => {
    ...
    // Note: CLAUDE_RECALL_GEMINI_RATE_LIMITING_ENABLED !== 'false' means enabled
