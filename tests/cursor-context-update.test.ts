 ...
 * .cursor/rules/claude-recall-context.mdc for registered projects.
 *
...
describe('Cursor Context Update', () => {
  ...
  describe('writeContextFile', () => {
    ...
    it('creates claude-recall-context.mdc file', () => {
      writeContextFile(workspacePath, 'test context');
    ...
    it('includes description in frontmatter', () => {
      ...
      const content = readContextFile(workspacePath);
      expect(content).toContain('description: "claude-recall context from past sessions (auto-updated)"');
    });
...
- Added new feature`;
    ...
    it('includes footer with MCP tools mention', () => {
      ...
      const content = readContextFile(workspacePath);
      expect(content).toContain("Use claude-recall's MCP search tools for more detailed queries");
    });

    it('uses atomic write (no temp file left behind)', () => {
      writeContextFile(workspacePath, 'test context');

      const tempFile = join(workspacePath, '.cursor', 'rules', 'claude-recall-context.mdc.tmp');
