 ...
 *
 * Tests the dual-tag privacy system for <private> and <claude-recall-context> tags.
 ...
 * - Privacy patterns from src/core/worker/http/routes/SessionRoutes.ts
 */
...
describe('Tag Stripping Utilities', () => {
  ...
  describe('stripMemoryTagsFromPrompt', () => {
    describe('basic tag removal', () => {
      ...
      it('should strip single <claude-recall-context> tag', () => {
        const input = 'public content <claude-recall-context>injected context</claude-recall-context> more public';
        const result = stripMemoryTagsFromPrompt(input);
        expect(result).toBe('public content  more public');
      });

      it('should strip both tag types in mixed content', () => {
        const input = '<private>secret</private> public <claude-recall-context>context</claude-recall-context> end';
        const result = stripMemoryTagsFromPrompt(input);
    ...
    describe('multiple tags handling', () => {
      ...
      it('should strip multiple <claude-recall-context> blocks', () => {
        const input = '<claude-recall-context>ctx1</claude-recall-context><claude-recall-context>ctx2</claude-recall-context> content';
        const result = stripMemoryTagsFromPrompt(input);
      ...
      it('should handle many interleaved tags', () => {
        let input = 'start';
        for (let i = 0; i < 10; i++) {
          input += ` <private>p${i}</private> <claude-recall-context>c${i}</claude-recall-context>`;
        }
        input += ' end';
        const result = stripMemoryTagsFromPrompt(input);
        // Tags are stripped but spaces between them remain
        expect(result).not.toContain('<private>');
        expect(result).not.toContain('<claude-recall-context>');
    ...
    describe('empty and private-only prompts', () => {
      ...
      it('should return empty string for entirely context-tagged prompt', () => {
        const input = '<claude-recall-context>all is context</claude-recall-context>';
        const result = stripMemoryTagsFromPrompt(input);
        expect(result).toBe('');
      });
      ...
      it('should handle whitespace-only after stripping', () => {
        const input = '<private>content</private>   <claude-recall-context>more</claude-recall-context>';
        const result = stripMemoryTagsFromPrompt(input);
        expect(result).toBe('');
      });
    });
...
secret
      ...
      it('should strip multiline content within <claude-recall-context> tags', () => {
        const input = `start
...
finish`;
  ...
  describe('stripMemoryTagsFromJson', () => {
    describe('JSON content stripping', () => {
      ...
      it('should strip claude-recall-context tags from JSON', () => {
        const jsonContent = JSON.stringify({
          data: '<claude-recall-context>injected</claude-recall-context> real data'
        });
        const result = stripMemoryTagsFromJson(jsonContent);
        const parsed = JSON.parse(result);
      ...
      it('should handle tool_response with tags', () => {
        const toolResponse = {
          output: 'result <claude-recall-context>context data</claude-recall-context>',
          status: 'success'
        };
