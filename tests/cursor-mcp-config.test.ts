...
describe('Cursor MCP Configuration', () => {
  ...
  const mcpServerPath = '/path/to/search-server.cjs';
  ...
  describe('configureCursorMcp', () => {
    ...
    it('adds claude-recall server with correct structure', () => {
      configureCursorMcp(mcpJsonPath, mcpServerPath);

      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));

      expect(config.mcpServers).toBeDefined();
      expect(config.mcpServers['claude-recall']).toBeDefined();
      expect(config.mcpServers['claude-recall'].command).toBe('node');
      expect(config.mcpServers['claude-recall'].args).toEqual([mcpServerPath]);
    });

    it('preserves existing MCP servers when adding claude-recall', () => {
    ...
    it('updates existing claude-recall server path', () => {
      // First config
      ...
      expect(config.mcpServers['claude-recall'].args).toEqual([newPath]);
    });
  ...
  describe('removeMcpConfig', () => {
    it('removes claude-recall server from config', () => {
      configureCursorMcp(mcpJsonPath, mcpServerPath);
      removeMcpConfig(mcpJsonPath);

      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(config.mcpServers['claude-recall']).toBeUndefined();
    });

    it('preserves other servers when removing claude-recall', () => {
      // Setup: both servers
      mkdirSync(join(tempDir, '.cursor'), { recursive: true });
      const config = {
        mcpServers: {
          'other-server': { command: 'python', args: ['/path.py'] },
          'claude-recall': { command: 'node', args: ['/mcp.cjs'] }
        }
      };
      ...
      expect(updated.mcpServers['claude-recall']).toBeUndefined();
    });
    ...
    it('does nothing if claude-recall not in config', () => {
      mkdirSync(join(tempDir, '.cursor'), { recursive: true });
      const config = {
        mcpServers: {
          'other-server': { command: 'python', args: ['/path.py'] }
  ...
  describe('path handling', () => {
    it('handles absolute path with spaces', () => {
      const pathWithSpaces = '/path/to/my project/search-server.cjs';
      configureCursorMcp(mcpJsonPath, pathWithSpaces);

      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(config.mcpServers['claude-recall'].args).toEqual([pathWithSpaces]);
    });

    it('handles Windows-style path', () => {
      const windowsPath = 'C:\\Users\\alex\\.claude\\plugins\\search-server.cjs';
      configureCursorMcp(mcpJsonPath, windowsPath);

      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(config.mcpServers['claude-recall'].args).toEqual([windowsPath]);
    });

    it('handles path with special characters', () => {
      const specialPath = "/path/to/project-name_v2.0 (beta)/search-server.cjs";
      configureCursorMcp(mcpJsonPath, specialPath);

      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(config.mcpServers['claude-recall'].args).toEqual([specialPath]);

      // Verify it survives JSON round-trip
      const reread: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(reread.mcpServers['claude-recall'].args![0]).toBe(specialPath);
    });
  });
});
